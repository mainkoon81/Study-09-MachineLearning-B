...
NN's each "node" is a linear model(classifier) whose weights(parameters) are updated by the optimization algorithm like a Gradient_Descent that minimizing the cost function(MSE, CrossEntropy, ), and this linear model(classifier) becomes the input of the activation function (step for discrete, sigmoid for continous) that returns the outcome of either classification/probability.  
